...
def compute_PR_RR(Agreement_list, input_signal_1, input_signal_2, fs_1, fs_2, show=False):
    ...
    # 0.25s
    peak_1 = signal.find_peaks(input_signal_1, distance=10)
    peak_2 = signal.find_peaks(input_signal_2, distance=10)
    PR = np.array([(x[1] - x[0]) * 1000 / fs_1 for x in zip(peak_1[0][:-1], peak_1[0][1:])])
    RR = np.array([(x[1] - x[0]) * 1000 / fs_2 for x in zip(peak_2[0][:-1], peak_2[0][1:])])
    # RR = mean_filter(RR, 5)
...
# Bland-Altman图一致性
print("{:.4f}".format(np.average(Agreement_list) * 100))  # 93.92
