...
from UBFC_Phys_Dataset_npy_limit import rPPG_Dataset_level, data_selected_level
import random
...
def train_epoch(net, device, data_loader, criterion_PHC, criterion_PH, optimizer, net_type):
    ...
    for i, data in enumerate(data_loader):
        _, tasks, level, bvp, peak, vpg, vpg_peak, HR, gaze, pose = data
        level, tasks, bvp, peak, HR = Variable(level).to(device), Variable(tasks).to(device), Variable(bvp).to(
        ...
        if net_type == "hr":
            ...
            loss = criterion_PHC(outputs, level, p_peak, peak, hr.squeeze(-1), HR)
            loss.backward()
            optimizer.step()
            _, predicted = torch.max(outputs, 1)
            train_loss += loss.item() * bvp.size(0)
            scores, predictions = torch.max(outputs.data, 1)
            train_correct += (predictions == level).sum().item()
            MAE += (hr.squeeze(-1) - HR).abs().mean()
    if net_type == "hr":
...
def valid_epoch(net, device, data_loader, criterion_PHC, criterion_PH, net_type):
    ...
    for i, data in enumerate(data_loader):
        ...
        if net_type == "hr":
            ...
            loss = criterion_PHC(output_test, level, p_peak, peak, hr.squeeze(-1), HR)
            _, predicted = torch.max(output_test, 1)
            valid_loss += loss.item() * bvp.size(0)
            scores, predictions = torch.max(output_test.data, 1)
            val_correct += (predictions == level).sum().item()
            MAE += (hr.squeeze(-1) - HR).abs().mean()

            TP += np.sum((level.detach().cpu().numpy() == 1) & (predicted.detach().cpu().numpy() == 1))
            TN += np.sum((level.detach().cpu().numpy() == 0) & (predicted.detach().cpu().numpy() == 0))
            FP += np.sum((level.detach().cpu().numpy() == 0) & (predicted.detach().cpu().numpy() == 1))
            FN += np.sum((level.detach().cpu().numpy() == 1) & (predicted.detach().cpu().numpy() == 0))
    if net_type == "hr":
...
if __name__ == '__main__':
    ...
    foldperf = {}
    person_list, tasks, levels = data_selected_level("./info")
    person_list, tasks, levels = np.array(person_list), np.array(tasks), np.array(levels)
    for fold, (train_idx, val_idx) in enumerate(splits.split(np.arange(len(person_list)), levels)):
        print('Fold {}'.format(fold + 1))

        batch_size = cfg['train']['batch_size']
        num_workers = cfg['train']['num_workers']

        train_p, train_t, train_l = person_list[train_idx], tasks[train_idx], levels[train_idx]
        val_p, val_t, val_l = person_list[val_idx], tasks[val_idx], levels[val_idx]
        ...
        train_dataset = rPPG_Dataset_level(train_p, train_t, train_l)
        test_dataset = rPPG_Dataset_level(val_p, val_t, val_l)
                ...
                if cfg['train'][
                    ...
                    torch.save(net.module,
                               os.path.join(cfg['train']['save_path'],
                                            'MTASR_level_pa{2}_hr{3}_{0}_{1}.pth'.format(fold + 1,
                                                                                         cfg['train']['clip_len'],
                                                                                         weight_pp, weight_hr)))
                elif cfg['train'][
